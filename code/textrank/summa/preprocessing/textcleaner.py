...
# Taken from gensim #todo: if sentence is to be respected, implement a sentence splitter, then within sentence, do tokenization. return a sequence of tokens and also a list indicating their sentence membership
...
def extract_word_from_corpus(in_corpus, out_file, language="english"):
    """ Tokenizes a given text into words, applying filters and lemmatizing them.
    Returns a dict of word -> syntacticUnit. """
    init_textcleanner(language)
    words=set()
    for file in os.listdir(in_corpus):
        with open(in_corpus+"/"+file, 'r') as myfile:
            ...
            units = merge_syntactic_units(original_words, filtered_words, tags)
            for unit in units:
                words.add(unit.text+"\t"+unit.token)
...
# extract_word_from_corpus("/home/zqz/Work/data/jate_data/genia_gs/text/files_standard",
#                          "/home/zqz/Work/data/texpr/corpus_words/words_genia.txt")
